...
namespace InsurenceManagementSystemWebApi.Infrastructure.Persistance.Repositories
{
    public class AgentRepository : IAgentRepository
    {
        ...
        public async Task<OperationResult<IEnumerable<Agent>>> GetAllAsync()
        {
            ...
            var agent= await _context.Agents.Include(u =>u.User).ToListAsync();

            return OperationResult<IEnumerable<Agent>>.Success(agent);
        }
        ...
        {
            var agent = await _context.Agents.Include(u=>u.User).FirstOrDefaultAsync(a => a.AgentId == agentId);
            if (agent == null)
            {
                return OperationResult<Agent>.Failure("Agent not exist");
            }
