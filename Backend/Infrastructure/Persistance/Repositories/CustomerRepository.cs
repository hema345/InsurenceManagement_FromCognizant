...
namespace InsurenceManagementSystemWebApi.Infrastructure.Persistance.Repositories
{
    public class CustomerRepository : ICustomerRepository
    {
        ...
        {
            var customer = await _context.Customers.Include(u => u.User).ToListAsync();
            return OperationResult<IEnumerable<Customer>>.Success(customer);
        }
        ...
        {
            var customer = await _context.Customers.Include(u=>u.User).FirstOrDefaultAsync(c => c.CustomerId == customerId);
            if (customer == null)
            {
                return OperationResult<Customer?>.Failure("Customer Not Found");
            }
